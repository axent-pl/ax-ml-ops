...
class ModelTrainer:
    ...
    def run(self, label:str, model_name:str, params:dict, features_class:str, scoring:str = 'accuracy'):
        ...
        with mlflow.start_run(run_name=label) as run:
            model = ModelTrain.train(model_name=model_name, x=x, y=y, params=params, scoring=scoring)
            model.get_serializer().log_model(model, artifact_path=label, registered_model_name=label)
